...
module Make (Str : STRING) = struct
  ...
  let edit_distance s1 s2 =
    if Str.length s1 = 0 then
      Str.length s2
    else if Str.length s2 = 0 then
      Str.length s1
    else if s1 = s2 then
      0
    else (
      ...
      for i = 0 to Str.length s1 - 1 do
        ...
        for j = 0 to Str.length s2 - 1 do
          let cost =
            if Str.compare_char (Str.get s1 i) (Str.get s2 j) = 0 then
              0
            else
              1
          in
  ...
  module NDA = struct
    ...
    module TrSet = Set.Make (struct
      ...
      let compare_char a b =
        match a, b with
        | Any, Any -> 0
        | Char c1, Char c2 -> Str.compare_char c1 c2
        | Any, Char _ -> -1
        | Char _, Any -> 1
      ...
      let compare a b =
        match a, b with
        | Success, Success -> 0
        | Upon (c1, i1, j1), Upon (c2, i2, j2) ->
          let c = compare_char c1 c2 in
        ...
        | Epsilon (i1, j1), Epsilon (i2, j2) ->
          if i1 <> i2 then
            Stdlib.compare i1 i2
          else
            Stdlib.compare j1 j2
        | _ -> Stdlib.compare (to_int_ a) (to_int_ b)
    end)

    (* non deterministic automaton *)
    type t = TrSet.t array array

    let[@inline] length (nda : t) = Array.length nda

    (* build NDA from the string *)
    let make ~limit s =
      let len = Str.length s in
      let m = Array.make_matrix (len + 1) (limit + 1) TrSet.empty in
      let add_transition i j tr =
        if not (TrSet.mem tr m.(i).(j)) then m.(i).(j) <- TrSet.add tr m.(i).(j)
      in
      ...
      for j = 0 to limit do
        ...
        if j < limit then add_transition len j (Upon (Any, len, j + 1));
        (* win in any case *)
        add_transition len j Success
      done;
      m

    let[@inline] get nda (i, j) = nda.(i).(j)
    let[@inline] is_final (nda : t) (i, j) = TrSet.mem Success (get nda (i, j))
  end
  ...
  module DFA = struct
    ...
    let add_state dfa =
      let n = dfa.len in
      (* resize *)
      if n = Array.length dfa.transitions then (
    ...
    let rec __mem_tr tr l =
      match tr, l with
      | _, [] -> false
      | (c, i), (c', i') :: l' -> (i = i' && compare c c' = 0) || __mem_tr tr l'

    (* add transition *)
    let add_transition dfa i tr =
      if not (__mem_tr tr dfa.transitions.(i)) then
        dfa.transitions.(i) <- tr :: dfa.transitions.(i)

    let add_otherwise dfa i j = dfa.otherwise.(i) <- j
    let set_final dfa i = dfa.is_final.(i) <- true
    ...
    let _set_to_string s =
      ...
      Buffer.add_char b '{';
      NDAStateSet.iter (fun (x, y) -> Printf.bprintf b "(%d,%d)" x y) s;
      Buffer.add_char b '}';
      Buffer.contents b

    (* list of characters that can specifically be followed from the given set *)
    let chars_from_set (nda : NDA.t) set =
      NDAStateSet.fold
        (fun state acc ->
          ...
          NDA.TrSet.fold
            (fun tr acc ->
              match tr with
              | NDA.Upon (NDA.Char c, _, _) -> CharSet.add c acc
              | _ -> acc)
            transitions acc)
        set CharSet.empty

    (* saturate current set w.r.t epsilon links *)
    let saturate_epsilon (nda : NDA.t) set =
      ...
      while not (Queue.is_empty q) do
        ...
        NDA.TrSet.iter
          (fun tr' ->
            match tr' with
            | NDA.Epsilon (i, j) ->
              if not (NDAStateSet.mem (i, j) !set) then Queue.push (i, j) q
            | _ -> ())
          (NDA.get nda state)
      done;
      !set
    ...
    let get_transition_for_char c acc transitions =
      NDA.TrSet.fold
        (fun tr acc ->
          match tr with
          | NDA.Upon (NDA.Char c', i, j) when Str.compare_char c c' = 0 ->
            (* follow same char *)
            NDAStateSet.add (i, j) acc
          | NDA.Upon (NDA.Any, i, j) ->
            (* follow '*' *)
            NDAStateSet.add (i, j) acc
          | _ -> acc)
        transitions acc

    let get_transitions_for_any acc transitions =
      NDA.TrSet.fold
        (fun tr acc ->
          match tr with
          | NDA.Upon (NDA.Any, i, j) -> NDAStateSet.add (i, j) acc
          | _ -> acc)
        transitions acc

    (* follow transition for given NDA.char, returns a new state
       and a boolean indicating whether it's final *)
    let follow_transition (nda : NDA.t) set c =
      let set' =
        NDAStateSet.fold
          (fun state acc ->
            let transitions = NDA.get nda state in
            (* among possible transitions, follow the one that matches c
               the most closely *)
            get_transition_for_char c acc transitions)
      ...
      let set' =
        NDAStateSet.fold
          (fun state acc ->
            ...
            get_transitions_for_any acc transitions)
          set NDAStateSet.empty
      in
      saturate_epsilon nda set'
    ...
    let iterate_transition_set (nda : NDA.t) set k =
    ...
    let rec traverse (nda : NDA.t) dfa states set =
      ...
      if is_final then set_final dfa set_i;
      iterate_transition_set nda set (fun c set' ->
    ...
    let get dfa i = dfa.transitions.(i)
    let otherwise dfa i = dfa.otherwise.(i)
    let is_final dfa i = dfa.is_final.(i)
  end
  ...
  let debug_print pp_char oc dfa =
    ...
    for i = 0 to dfa.DFA.len - 1 do
      ...
      if DFA.is_final dfa i then Printf.fprintf oc "  success %d\n" i;
      List.iter
        (fun (c, j) -> Printf.fprintf oc "  %d --%a--> %d\n" i pp_char c j)
        transitions;
      let o = DFA.otherwise dfa i in
      if o >= 0 then Printf.fprintf oc "  %d --*--> %d\n" i o
    done
  ...
  let of_list ~limit l = of_string ~limit (Str.of_list l)

  let rec __find_char c l =
    match l with
    | [] -> raise Not_found
    | (c', next) :: l' ->
      if compare c c' = 0 then
        next
      else
        __find_char c l'
  ...
  module Index = struct
    ...
    type 'b t = Node of 'b option * 'b t M.t
    ...
    let goto_leaf s node k =
      ...
      let rec goto node i rebuild =
        ...
        | Node (opt, m) ->
          ...
          let t' = try M.find c m with Not_found -> empty in
          goto t' (i + 1) (fun t'' ->
              if is_empty t'' then
                rebuild (Node (opt, M.remove c m))
              else
                rebuild (Node (opt, M.add c t'' m)))
      in
      goto node 0 (fun t -> t)

    let add trie s value =
      goto_leaf s trie (function Node (_, m) -> Node (Some value, m))

    let remove trie s =
      goto_leaf s trie (function Node (_, m) -> Node (None, m))
    ...
    let of_list l = List.fold_left (fun acc (arr, v) -> add acc arr v) empty l

    let fold f acc idx =
      let rec explore acc trail node =
        match node with
        | Node (opt, m) ->
          ...
          let acc =
            ...
            | Some v ->
              ...
              f acc str v
          in
          M.fold (fun c node' acc -> explore acc (c :: trail) node') m acc
      in
      explore acc [] idx

    let iter f idx = fold (fun () str v -> f str v) () idx
    let to_list idx = fold (fun acc str v -> (str, v) :: acc) [] idx
    ...
    let to_seq idx =
      let rec traverse node trail ~(fk : (string_ * 'a) Seq.t) () =
        ...
        | Node (opt, m) ->
          (* all alternatives: continue exploring [m], or call [fk] *)
          let fk =
            M.fold (fun c node' fk -> traverse node' (c :: trail) ~fk) m fk
          in
          (match opt with
          | Some v ->
            let str = Str.of_list (List.rev trail) in
