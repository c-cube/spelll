...
let test_automaton =
  ...
  let test (s, a) = Spelll.match_with a s in
  ...
  let test (l, idx) =
    List.for_all
      (fun s ->
        ...
        List.for_all (fun s' -> Spelll.edit_distance s s' <= 2) retrieved)
      l
  in
  let name =
    "strings retrieved from automaton with limit:n are at distance <= n"
  in
  QCheck.Test.make ~name ~long_factor:5 ~count:100 gen test
  ...
  let idx =
    lazy
      (let idx = ref Spelll.Index.empty in
       ...
       let add x =
         l := x :: !l;
         idx := Spelll.Index.add !idx x x
       in
       for i = 0 to 5_000 do
         for j = 0 to 40 do
           List.iter
             (fun s -> add @@ Printf.sprintf "foo-%d-%s-%d-bar" i s j)
             [ ""; "a"; "b"; "c"; "d" ]
         done
       done;
       !l, !idx)
...
let suite = [ test_index; test_mutation; test_automaton; test_big ]
let () = exit @@ QCheck_runner.run_tests ~verbose:true ~colors:true suite
